...
## Vendure E-commerce Skills

Skills for developing Vendure e-commerce plugins, extending GraphQL APIs, building Admin UI components, and defining database entities.

### `vendure-developing` - Core Vendure Development

**When to use:** Entry point for any Vendure development task

**What it does:**

- Provides quick reference for Vendure patterns
- Guides to vendure-expert agent for comprehensive support
- Documents forbidden and required patterns
- Links to domain-specific skills

**Related Agent:** Use `vendure-expert` agent for coordinated multi-domain guidance.

---

### `vendure-expert` - Vendure Expert Agent

**When to use:** Creating plugins, extending APIs, building Admin UI, or reviewing Vendure code

**What it does:**

- Coordinates 9 specialized Vendure skills
- Provides full plugin lifecycle guidance
- Routes to appropriate domain skill based on task
- Enforces Vendure best practices
...
### Vendure Domain Skills (Writing + Reviewing Pairs)

#### Plugin Development

- **`vendure-plugin-writing`** - Create plugins with @VendurePlugin, NestJS DI, lifecycle hooks
- **`vendure-plugin-reviewing`** - Audit plugins for best practices violations

#### GraphQL API

- **`vendure-graphql-writing`** - Extend Shop/Admin APIs, write resolvers with RequestContext
- **`vendure-graphql-reviewing`** - Review resolvers for security and patterns

#### Database/Entities

- **`vendure-entity-writing`** - Define VendureEntity models with TypeORM, migrations
- **`vendure-entity-reviewing`** - Audit entity patterns, relations, custom fields

#### Admin UI

- **`vendure-admin-ui-writing`** - Build React Admin UI extensions with UI DevKit
- **`vendure-admin-ui-reviewing`** - Review components for accessibility and patterns

#### Specialized

- **`vendure-delivery-plugin`** - Delivery/shipping patterns (ShippingCalculator, FulfillmentHandler, time slots)
